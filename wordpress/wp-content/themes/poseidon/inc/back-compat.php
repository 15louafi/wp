...
function poseidon_compat_upgrade_notice() {
	$message = sprintf( esc_html__( 'Poseidon requires at least WordPress version 4.4. You are running version %s. Please upgrade and try again.', 'poseidon' ), $GLOBALS['wp_version'] );
...
function poseidon_compat_customize() {
	wp_die( sprintf( esc_html__( 'Poseidon requires at least WordPress version 4.4. You are running version %s. Please upgrade and try again.', 'poseidon' ), $GLOBALS['wp_version'] ), '', array(
		'back_link' => true,
	) );
}
...
function poseidon_compat_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_html__( 'Poseidon requires at least WordPress version 4.4. You are running version %s. Please upgrade and try again.', 'poseidon' ), $GLOBALS['wp_version'] ) );
	}
}
