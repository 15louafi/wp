	...
	public static function _action_widgets_init() {
		...
		foreach ( $paths as $path ) {
			...
			foreach ( $dirs as $dir ) {
				...
				self::include_isolated( $dir . '/class-saiph-widget-' . $dirname . '.php' );

				$widget_class = 'Saiph_Widget_' . self::dirname_to_classname( $dirname );
