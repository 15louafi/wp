...
class Saiph_Widget_Blog_Tabs extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( false, __( 'Blog Tabs', 'saiph-lite' ), $widget_ops );
	}
	...
	public function widget( $args, $instance ) {
		...
		$this->saiph_theme_action_add_static();
	...
	public function fw_get_posts_with_info( $args = array() ) {
		...
		foreach ( $result_posts as $post ) {
			...
			$posts_with_info[ $post->ID ]['post_date']  = strtotime($post->post_date);
		}
	 ...
	 * @internal
	 */
	public function saiph_theme_action_add_static() {
		wp_enqueue_script('saiph-tabcollapse');
		wp_enqueue_script('saiph-velocity');
	}
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of Blog posts', 'saiph-lite' ); ?>
				:</label>
