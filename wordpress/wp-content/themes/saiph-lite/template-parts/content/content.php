...
$post_class        = 'tpl-blog-box ';
if ( $verticalFormat ) {
	$post_class .= 'vertical-format';
}
...
<article id="post-<?php the_ID(); ?>" <?php post_class( $post_class ); ?> itemscope=""
	...
	<?php if ( has_post_thumbnail() ) { ?>
		...
		<div class="tpl-blog-image">
			...
			<?php
			if ( $verticalFormat ) {
				the_post_thumbnail( 'saiph-vertical-thumb ' );
			} else {
				the_post_thumbnail( 'saiph-theme-full-width' );
			}
			?>
	...
	<?php } else { ?>
		...
		<div class="entry-content tpl-blog-post-body" itemprop="articleBody">
			<?php echo '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . __('Permalink', 'saiph-lite') . '</a>'?>
			<?php
			// Add the content
